...
def getPlateNumber(img):    
    ...
    plateno=[]
    for possiblePlate in plates:
        tmpplt=imagetotext.get_string(possiblePlate.imgPlate)
        plt=""
        print("tmp:",tmpplt)
        for i in tmpplt:
            if(bool(re.match('^[a-zA-Z0-9]+$', i))):
                plt+=i
        print("plt:",plt)
        if(len(plt)):
            plateno.append(plt)
        
    plateno=Sorting(plateno)
    print(plateno)
    return str(plateno[0])